        ...
        *,
        ...
        for symbol in active_symbols:
            ...
            entries[symbol] = WatchlistEntry(
                ...
                stage_scores=dict(candidate.stage_scores if candidate else {}),
                stage_components=dict(candidate.stage_components if candidate else {}),
                exclusion_reasons=list(candidate.exclusion_reasons if candidate else []),
                near_miss_reasons=list(candidate.near_miss_reasons if candidate else []),
                signals=list(candidate.signals if candidate else []),
                best_signal=dict(candidate.best_signal) if candidate and candidate.best_signal else None,
                opportunity_scores=list(candidate.opportunity_scores if candidate else []),
                rank_reason=candidate.rank_reason if candidate else None,
                relative_ranking_reason=candidate.relative_ranking_reason if candidate else None,
