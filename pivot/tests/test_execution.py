...
def ready(at=NOW, direction='long'):
    return dict(analysis_at=at.isoformat(), data_valid_until=(at+timedelta(seconds=90)).isoformat(), feeds={'vix': 'current'}, data_errors=[],
        setup=dict(state='SETUP_READY', checks=[dict(name=n, passed=True) for n in CHECKS],
                   policy_version='nasdaq-video-interpretation-v1', event_at=at.isoformat(),
