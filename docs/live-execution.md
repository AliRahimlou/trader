...
This document records the original execution release. Some details below are historical: the current leader rule is a four-of-seven majority with up to one opposing (a conflicting leader is neutral since 4.4.0) and a 60-minute reaction window, AllSpark hosts the worker, VIX has time-based retries within its allowance, the entry cutoff is 30 minutes before the close, and a rejected stock order pauses Socrates only instead of switching global Live off. See the [4.5.0 release note](production-v4.5.0.md) for the current rule interpretations and the [4.4.0 release note](production-v4.4.0.md) for the reward-to-risk, VIX-persistence and allowance rules it builds on.

**Short proxy since 4.5.0.** The statements below that a short "requires whole shares, account permission and borrow availability" and is otherwise skipped describe QQQ shorts, which this cash account still cannot execute. A confirmed Socrates short is now executed as a fractional **long purchase of PSQ**, the −1× daily inverse Nasdaq-100 ETF, for the saved dollar amount: the QQQ plan's stop and target are translated to PSQ by percentage, measured from the live QQQ bid at admission, and enforced on the PSQ price; the same fresh-quote, spread, price-movement, submission-budget, protection and exit rules apply; and the position is intraday only, flattened in the five-minute closing window and never held overnight. PSQ tracks the inverse daily return with tracking error, resets daily and has its own (thinner) quote, so the translated levels are approximate and a wide or stale PSQ quote can block an entry the QQQ setup admitted. One Socrates position at a time applies across QQQ and PSQ. This is an owner-accepted instrument translation; the recordings say only "Nasdaq short". See the [current method specification](research/current-method-specifications.md), [3.2 monitoring changes](production-v3.2.md), and [3.2.2 recovery audit](production-v3.2.2.md) for the current implementation and validation limits.
