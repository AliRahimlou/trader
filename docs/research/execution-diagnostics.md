...
Each record identifies its observation time, app version/build, execution stage and outcome, signal context, and trade/operation context when available. Exception categories are retained without exception text, provider responses, credentials or account identity. Stable repeated checks share one row with first/last timestamps and an observation count. The store retains up to 24,000 distinct states. Poll counts are not independent opportunities.
