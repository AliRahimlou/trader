...
| Event life | Origin bar to the **end of the next regular session**; missing intervening hourly bar still invalidates; one entry per event; entry deadlines unchanged. | Origin + 180 minutes, current session only. | Retests pointed at one to three days after the break; no limit stated. Two sessions is an app bound. |
| Leader areas | Union of 5-minute repeated-interaction zones (unchanged) and zero-width **period levels** from the company's regular-session data: previous day high/low/close, session open, previous week high/low/mid, Monday high/low, week open, month open, session VWAP. Same close-beyond reaction inequalities at either kind. | Interaction zones only. | Indicator-plotted period levels plus supply/demand boxes on AAPL/NVDA/MSFT; boxes and weekly/monthly VWAP are not reproduced. |
