...
Specification date: September 22, 2026. The definitions above remain the v3/4.3 record. Version 4.4.0 ([release note](../production-v4.4.0.md)) replaced the numerical interpretations below after the September 21 replay found that the 4.3 values admitted almost nothing (median target 0.52 R; VIX gate true on 28% of bars at random; one conflicting leader vetoing seven). Each is an app choice behind a new policy version and requires owner re-review; none is a formula the recordings state. Values marked `<<…>>` are filled from the committed constants at the tagged revision.
...
| Minimum reward-to-risk | Plan is skipped unless (target − entry) / (entry − stop) ≥ **<<MIN_RR>>** (reversed for a short). Stop formula unchanged. Nothing is resized or re-targeted to reach the ratio. | No minimum; median 0.52 R over 223 replayed plans. |
| VIX areas | Repeated swing extremes from completed fifteen-minute actual-VIX candles, excluding the two latest bars, with tolerance **<<VIX_TOL>>** of the VIX level. | 0.1% fixed band (≈0.015 points at VIX 15; median bar range 0.656%). |
| VIX reaction | Reaction candle closes on the reaction side of the area; the next **<<VIX_PERSIST>>** completed candle(s) must not close back through it. Invalidated by a later close through the area or by the 180-minute event window. Direction opposite the QQQ trade, as before. Fresh actual-index quote before entry unchanged. | Single latest candle, no persistence, no invalidation. |
...
| Crypto cost gate | Net reward after 0.25%-per-side fees and the execution allowance ≥ **<<CRYPTO_K>>** × net risk; the skip reason exposes fees, gross/net reward, net risk and ratio. Stop and 2 R gross target unchanged. | Positive net target only (admitted ≈ 0 net reward). |
| Crypto opening range | Valid with at least **<<OPENING_MIN>>** of 48 five-minute bars present between 00:00 and 04:00 ET, first and last bars present, bounded gaps; range from present bars. Confirmation still requires native closes; missing bars during confirmation void that excursion only. | Any missing bar voided the day. |
| VIX retries | Time-based retries at fixed offsets after a period boundary until the candle arrives, bounded per period, within the same durable allowance. | One attempt plus one recovery per period (up to 14-minute waits). |
| Versions | Socrates policy **<<QQQ_POLICY_VERSION>>**; crypto rule **<<RANGE_RULE_VERSION>>**; crypto policy **<<CRYPTO_POLICY_VERSION>>**. Event identities carry the rule version. | `nasdaq-qqq-execution-v5-five-leader-majority`, `range-reversal-v1`, `range-spot-execution-v1`. |

Deferred: an inverse-ETF proxy for Socrates shorts (the account cannot short and $15 cannot form a whole QQQ share) and an afternoon four-hour crypto bucket (not a stated source rule; replays are net negative after fees). The research candidates F1 and X1 below are unaffected and remain isolated from production.
