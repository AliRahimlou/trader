...
| Event life | Origin bar to the **end of the next regular session**; missing intervening hourly bar still invalidates; one entry per event; entry deadlines unchanged. After the retest the plan is rebuilt on each completed hourly candle until expiry with the latest hourly close as entry reference; no proximity to the area is required (unchanged 4.4 mechanics, now over two sessions). Target levels must still predate the origin bar, so an entry on the next session cannot target the break day's previous-day high/low or four-hour areas established after the break. | Origin + 180 minutes, current session only. | Retests pointed at one to three days after the break; no limit stated. Two sessions is an app bound. |
